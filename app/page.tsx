...
export default async function Page() {
  ...
  return <main className="shell">
    ...
    <section className="grid2">
      <Section title="portfolio risk" caption={holdings.length ? `${holdings.length} active holdings` : 'cash / no active holdings'} icon={<WalletCards size={17}/>}>
        {holdings.length ? <div className="opps">
          {holdings.map((holding: any) => <article className="opportunityCard" key={holding.ticker}>
            <div className="oppHead"><div><span className="ticker">{holding.ticker}</span><span className="sector">{holding.shares ?? 0} shares · avg {formatPkr(holding.avg_cost_pkr)}</span></div><Pill value={holding.risk_action}>{holding.risk_action || 'n/a'}</Pill></div>
            <div className="rows tight">
              <Row label="current/rules price" value={formatPkr(holding.price)} />
              <Row label="market value" value={formatPkr(holding.market_value_pkr)} />
              <Row label="unrealized p&l" value={`${formatPkr(holding.unrealized_pnl_pkr)} · ${formatPct(holding.unrealized_pnl_pct)}`} />
              <Row label="portfolio weight" value={formatPct(holding.portfolio_weight_pct)} />
              <Row label="risk score" value={`${holding.risk_score ?? 'n/a'}/100`} toneValue={holding.risk_action} />
            </div>
          </article>)}
